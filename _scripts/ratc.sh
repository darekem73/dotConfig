...
ARG="$1"
function usage() {
	echo "Usage: <$0> { <preset_number> | --auto | --random | --next | --prev | --choice | --restore | --reset | --list }"
}
...
if [[ -f "$ARG" ]]; then
	...
	SCHEME=themes/`ls -1 $HOME/$CONF_DIR/themes | shuf | head -1`
